...
EOT;
                                ...
                                // see https://www.postgresql.org/docs/9.4/static/datatype-numeric.html#DATATYPE-SERIAL
                                // yes that is exactly what you see here, sir
                                // duplicate code for generating sequence name like PostrgresDialect::makeSequenceName, because we have no DBTable or DBColumn and too lazy to pass params or some other shit
                                $sequenceName = $class->getTableName() . '_' . $class->getIdentifier()->getColumnName();
                                if ($class->getIdentifier()->getType()->isSerial()) {
                                    $sequenceName .= '_seq';
                                }
				$out .= <<<EOT
