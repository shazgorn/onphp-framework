...
abstract class /* ANSI's */ Dialect
{
    ...
    public function defaultToString(DBColumn $column)
    {
        if ($column->getType()->getId() == DataType::BOOLEAN) {
            $default = $column->getDefault()
                ? $this->literalToString(Dialect::LITERAL_TRUE)
                : $this->literalToString(Dialect::LITERAL_FALSE);
        } else {
            $default = $this->valueToString($column->getDefault());
        }
        return $default;
    }
